...
from PyQt6.QtWidgets import (
    ...
    QAbstractItemView, QDialog, QDialogButtonBox, QTextBrowser
)
...
class DictionaryDatabase:
    ...
    def init_database(self):
        ...
        if cursor.fetchone()[0] == 0:
            sample_data = [
                ...
                ("goodbye", "លាហើយ", "greeting", "A farewell expression", "Goodbye see you tomorow!" ),
                ("thank you", "អរគុណ", "expression", "Expresstion of gratitude", "Thank you for your help"),
                ("please", "សូម", "adverb", "Used to make a polite request","Please help me" ),
                ("yes", "បាទ/ចាស", "response", "affirmative response", "Yes, I agree"),
                ("no", "ទេ", "response", "negative response", "No, I dont't want"),
                ("water", "ទឹក", "noun", "Clear liquid essential for life", "I need water"),
                ("food", "អាហារ", "noun", "Substance consumed for nutrition", "The food is deliclious"),
                ...
                ("mobile", "ចល័ត", "verb", " anywhere", " go to everywhere"),
                ("TV", "ទូរទស្សន៍", "noun", "Television", "she watched TV so long")
            ]
    ...
    def create_word(self, englsih_word, khmer_word, word_type="noun", definition="", example=""):
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO dictionary (english_word, khmer_word, word_type, definition, example_sentence)
                VALUES(?, ?, ?, ?, ?)
            ''', (englsih_word.lower().strip(), khmer_word.strip(), word_type, definition, example))
            conn.commit()
    ...
    def delete_word(self, word_id):
        """DELETE operarion - Remove word from dictionary"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        try:
...
class WordDetailsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        if self.word_data:
            ...
            info_text = f"""
                <h1 style='color: # 2E7D32;'>{english.title()} ↔ {khmer}</h1>
                <p><strong>Word ID: </strong>{word_type}</p>
...
class TranslatorWidget(QWidget):
    ...
    def init_ui(self):
        ...
        subtitle = QLabel("វចនានុក្រមអង់គ្លេស-ខ្មែរ")
    ...
    def search_word(self):
        ...
        if results:
            ...
            no_results = f"""
                ...
                <p><strong>Suggetions:</strong></p>
                <ul>
                    <li>Check spelling</li>
                    <li>Try simple words</li>
                    <li>Use th Dictionary Manger to add new words</li>
                </ul>
            """
    ...
    def display_results(self, results):
        ...
        for result in results:
            word_id, english, khmer, word_type, definition, example, created, update = result
            ...
            html_content += f"""
                ...
                {f"<p><strong>Example: </strong> <em>{example}</p>" if example else ""}
            </div>
            """
...
class DictionaryManageWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # CREATE form input using QHBoxLayout
        ...
        # Add all input groups to horizontation
...
class StatisticsWidget(QWidget):
    ...
    def update_stats(self):
        ...
        html_countent = "<h3> Sample Words: </h3>"
        for word in sample_words:
            english, khmer = word[1], word[2]
            html_countent += f"<p><strong>{english}</strong> ➡ {khmer}</p>"

        self.sample_display.setHtml(html_countent)
...
class KhmerEnglishDictionaryApp(QMainWindow):
    ...
    def connect_signals(self):
        ...
        self.manager_tab.word_added.connect(
            lambda word: self.statusBar().showMessage(f"✔️ Creates word: {word}")
        )
        
        self.manager_tab.word_updated.connect(
            lambda word_id: self.statusBar().showMessage(f"✔️ Update  word ID: {word_id}")
        )
        
        self.manager_tab.word_deleted.connect(
            lambda word_id: self.statusBar().showMessage(f"✔️ Delete word ID: {word_id}")
        )
...
def main():
    ...
    app.setOrganizationName("Studet Learing Tools")
